 ...
 */
public class OrmliteLibraryHelper extends OrmLiteSqliteOpenHelper {
    // 数据库名称
    private static String DATABASE_NAME = "baseData.db";

    public final static int version = 1;
    // 本类的单例实例
    private static OrmliteLibraryHelper instance;

    // 存储APP中所有的DAO对象的Map集合
    private Map<String, Dao> daos = new HashMap<>();

    // 获取本类单例对象的方法
    public static synchronized OrmliteLibraryHelper getInstance(Context context) {
        if (instance == null) {
            synchronized (OrmliteLibraryHelper.class) {
                if (instance == null) {
                    instance = new OrmliteLibraryHelper(context);
                }
            }
        }
        return instance;
    ...
    // 私有的构造方法
    private OrmliteLibraryHelper(Context context) {
