...
public abstract class BaseVolleyApplication extends MyBaseApplication {
    /**
    ...
    /**
     * @return true, 开启volley  ,false 不开启volley
     */
    protected abstract boolean isOpenVolley();

    /**
     * 是否开启Volley的重试请求策略
     * @return  是否开启重试请求策略
     */
    protected abstract boolean isOpenRetryPolicy();
    ...
    public void volleyGet(String url, final VolleyListener listener, final int flag) {
        if (!isOpenVolley() || !MyNetWorkUtils.isNetworkAvailable(getApplicationContext())) {
            return;
        }
