package com.xuanyuan.arrlibrary;
 ...
 *
 * 使用方法：点击调用   mLikeViewLayout.addLikeView()方法，连续点击或单击都会有小爱心出现
 *
 *        <com.dong.demo.FlowLikeView
...
public class FlowLikeView extends RelativeLayout {
    ...
    private ValueAnimator generateCurveAnimation(View target) {
        ...
        ValueAnimator valueAnimator = ValueAnimator.ofObject(evaluator,
                new PointF((float) ((mViewWidth - mPicWidth) / 2), mViewHeight - mChildViewHeight - mPicHeight),
                new PointF((float) (mViewWidth / 2) + (mRandom.nextBoolean() ? 1 : -1) * mRandom.nextInt(100), 0));
    ...
    private PointF generateCTRLPointF(int value) {
        ...
        pointF.x = (float) (mViewWidth / 2) - mRandom.nextInt(100);
    ...
    private class CurveEvaluator implements TypeEvaluator<PointF> {
        ...
        CurveEvaluator(PointF ctrlPointF1, PointF ctrlPointF2) {
...
//                    + ((float) Math.pow(fraction, 2)) * endValue.y;
    ...
    private class AnimationEndListener extends AnimatorListenerAdapter {
        ...
        AnimationEndListener(View target) {
            this.target = target;
        }
